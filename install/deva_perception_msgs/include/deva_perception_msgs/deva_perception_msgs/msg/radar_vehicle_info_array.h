// generated from rosidl_generator_c/resource/idl.h.em
// with input from deva_perception_msgs:msg/RadarVehicleInfoArray.idl
// generated code does not contain a copyright notice

#ifndef DEVA_PERCEPTION_MSGS__MSG__RADAR_VEHICLE_INFO_ARRAY_H_
#define DEVA_PERCEPTION_MSGS__MSG__RADAR_VEHICLE_INFO_ARRAY_H_

#include "deva_perception_msgs/msg/detail/radar_vehicle_info_array__struct.h"
#include "deva_perception_msgs/msg/detail/radar_vehicle_info_array__functions.h"
#include "deva_perception_msgs/msg/detail/radar_vehicle_info_array__type_support.h"

#endif  // DEVA_PERCEPTION_MSGS__MSG__RADAR_VEHICLE_INFO_ARRAY_H_
