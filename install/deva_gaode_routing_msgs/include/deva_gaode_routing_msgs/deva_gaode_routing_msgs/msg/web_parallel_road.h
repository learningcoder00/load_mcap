// generated from rosidl_generator_c/resource/idl.h.em
// with input from deva_gaode_routing_msgs:msg/WebParallelRoad.idl
// generated code does not contain a copyright notice

#ifndef DEVA_GAODE_ROUTING_MSGS__MSG__WEB_PARALLEL_ROAD_H_
#define DEVA_GAODE_ROUTING_MSGS__MSG__WEB_PARALLEL_ROAD_H_

#include "deva_gaode_routing_msgs/msg/detail/web_parallel_road__struct.h"
#include "deva_gaode_routing_msgs/msg/detail/web_parallel_road__functions.h"
#include "deva_gaode_routing_msgs/msg/detail/web_parallel_road__type_support.h"

#endif  // DEVA_GAODE_ROUTING_MSGS__MSG__WEB_PARALLEL_ROAD_H_
