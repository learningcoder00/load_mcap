// generated from rosidl_typesupport_fastrtps_c/resource/idl__type_support_c.cpp.em
// with input from deva_gaode_routing_msgs:msg/Restriction.idl
// generated code does not contain a copyright notice
#include "deva_gaode_routing_msgs/msg/detail/restriction__rosidl_typesupport_fastrtps_c.h"


#include <cassert>
#include <limits>
#include <string>
#include "rosidl_typesupport_fastrtps_c/identifier.h"
#include "rosidl_typesupport_fastrtps_c/wstring_conversion.hpp"
#include "rosidl_typesupport_fastrtps_cpp/message_type_support.h"
#include "deva_gaode_routing_msgs/msg/rosidl_typesupport_fastrtps_c__visibility_control.h"
#include "deva_gaode_routing_msgs/msg/detail/restriction__struct.h"
#include "deva_gaode_routing_msgs/msg/detail/restriction__functions.h"
#include "fastcdr/Cdr.h"

#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-parameter"
# ifdef __clang__
#  pragma clang diagnostic ignored "-Wdeprecated-register"
#  pragma clang diagnostic ignored "-Wreturn-type-c-linkage"
# endif
#endif
#ifndef _WIN32
# pragma GCC diagnostic pop
#endif

// includes and forward declarations of message dependencies and their conversion functions

#if defined(__cplusplus)
extern "C"
{
#endif

#include "deva_gaode_routing_msgs/msg/detail/restriction_detail__functions.h"  // restriction_detail
#include "rosidl_runtime_c/primitives_sequence.h"  // pass_list
#include "rosidl_runtime_c/primitives_sequence_functions.h"  // pass_list
#include "rosidl_runtime_c/string.h"  // toll_lane_type, toll_mode, toll_name
#include "rosidl_runtime_c/string_functions.h"  // toll_lane_type, toll_mode, toll_name

// forward declare type support functions
size_t get_serialized_size_deva_gaode_routing_msgs__msg__RestrictionDetail(
  const void * untyped_ros_message,
  size_t current_alignment);

size_t max_serialized_size_deva_gaode_routing_msgs__msg__RestrictionDetail(
  bool & full_bounded,
  bool & is_plain,
  size_t current_alignment);

const rosidl_message_type_support_t *
  ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, deva_gaode_routing_msgs, msg, RestrictionDetail)();


using _Restriction__ros_msg_type = deva_gaode_routing_msgs__msg__Restriction;

static bool _Restriction__cdr_serialize(
  const void * untyped_ros_message,
  eprosima::fastcdr::Cdr & cdr)
{
  if (!untyped_ros_message) {
    fprintf(stderr, "ros message handle is null\n");
    return false;
  }
  const _Restriction__ros_msg_type * ros_message = static_cast<const _Restriction__ros_msg_type *>(untyped_ros_message);
  // Field name: id
  {
    cdr << ros_message->id;
  }

  // Field name: limit_type
  {
    cdr << ros_message->limit_type;
  }

  // Field name: node_id
  {
    cdr << ros_message->node_id;
  }

  // Field name: pass_list
  {
    size_t size = ros_message->pass_list.size;
    auto array_ptr = ros_message->pass_list.data;
    cdr << static_cast<uint32_t>(size);
    cdr.serializeArray(array_ptr, size);
  }

  // Field name: out_road
  {
    cdr << ros_message->out_road;
  }

  // Field name: passage
  {
    cdr << ros_message->passage;
  }

  // Field name: slope
  {
    cdr << ros_message->slope;
  }

  // Field name: toll_mode
  {
    const rosidl_runtime_c__String * str = &ros_message->toll_mode;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: toll_lane_type
  {
    const rosidl_runtime_c__String * str = &ros_message->toll_lane_type;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: toll_name
  {
    const rosidl_runtime_c__String * str = &ros_message->toll_name;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: restriction_detail
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
      ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
        rosidl_typesupport_fastrtps_c, deva_gaode_routing_msgs, msg, RestrictionDetail
      )()->data);
    if (!callbacks->cdr_serialize(
        &ros_message->restriction_detail, cdr))
    {
      return false;
    }
  }

  return true;
}

static bool _Restriction__cdr_deserialize(
  eprosima::fastcdr::Cdr & cdr,
  void * untyped_ros_message)
{
  if (!untyped_ros_message) {
    fprintf(stderr, "ros message handle is null\n");
    return false;
  }
  _Restriction__ros_msg_type * ros_message = static_cast<_Restriction__ros_msg_type *>(untyped_ros_message);
  // Field name: id
  {
    cdr >> ros_message->id;
  }

  // Field name: limit_type
  {
    cdr >> ros_message->limit_type;
  }

  // Field name: node_id
  {
    cdr >> ros_message->node_id;
  }

  // Field name: pass_list
  {
    uint32_t cdrSize;
    cdr >> cdrSize;
    size_t size = static_cast<size_t>(cdrSize);

    // Check there are at least 'size' remaining bytes in the CDR stream before resizing
    auto old_state = cdr.getState();
    bool correct_size = cdr.jump(size);
    cdr.setState(old_state);
    if (!correct_size) {
      fprintf(stderr, "sequence size exceeds remaining buffer\n");
      return false;
    }

    if (ros_message->pass_list.data) {
      rosidl_runtime_c__uint64__Sequence__fini(&ros_message->pass_list);
    }
    if (!rosidl_runtime_c__uint64__Sequence__init(&ros_message->pass_list, size)) {
      fprintf(stderr, "failed to create array for field 'pass_list'");
      return false;
    }
    auto array_ptr = ros_message->pass_list.data;
    cdr.deserializeArray(array_ptr, size);
  }

  // Field name: out_road
  {
    cdr >> ros_message->out_road;
  }

  // Field name: passage
  {
    cdr >> ros_message->passage;
  }

  // Field name: slope
  {
    cdr >> ros_message->slope;
  }

  // Field name: toll_mode
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->toll_mode.data) {
      rosidl_runtime_c__String__init(&ros_message->toll_mode);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->toll_mode,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'toll_mode'\n");
      return false;
    }
  }

  // Field name: toll_lane_type
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->toll_lane_type.data) {
      rosidl_runtime_c__String__init(&ros_message->toll_lane_type);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->toll_lane_type,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'toll_lane_type'\n");
      return false;
    }
  }

  // Field name: toll_name
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->toll_name.data) {
      rosidl_runtime_c__String__init(&ros_message->toll_name);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->toll_name,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'toll_name'\n");
      return false;
    }
  }

  // Field name: restriction_detail
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
      ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
        rosidl_typesupport_fastrtps_c, deva_gaode_routing_msgs, msg, RestrictionDetail
      )()->data);
    if (!callbacks->cdr_deserialize(
        cdr, &ros_message->restriction_detail))
    {
      return false;
    }
  }

  return true;
}  // NOLINT(readability/fn_size)

ROSIDL_TYPESUPPORT_FASTRTPS_C_PUBLIC_deva_gaode_routing_msgs
size_t get_serialized_size_deva_gaode_routing_msgs__msg__Restriction(
  const void * untyped_ros_message,
  size_t current_alignment)
{
  const _Restriction__ros_msg_type * ros_message = static_cast<const _Restriction__ros_msg_type *>(untyped_ros_message);
  (void)ros_message;
  size_t initial_alignment = current_alignment;

  const size_t padding = 4;
  const size_t wchar_size = 4;
  (void)padding;
  (void)wchar_size;

  // field.name id
  {
    size_t item_size = sizeof(ros_message->id);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name limit_type
  {
    size_t item_size = sizeof(ros_message->limit_type);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name node_id
  {
    size_t item_size = sizeof(ros_message->node_id);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name pass_list
  {
    size_t array_size = ros_message->pass_list.size;
    auto array_ptr = ros_message->pass_list.data;
    current_alignment += padding +
      eprosima::fastcdr::Cdr::alignment(current_alignment, padding);
    (void)array_ptr;
    size_t item_size = sizeof(array_ptr[0]);
    current_alignment += array_size * item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name out_road
  {
    size_t item_size = sizeof(ros_message->out_road);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name passage
  {
    size_t item_size = sizeof(ros_message->passage);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name slope
  {
    size_t item_size = sizeof(ros_message->slope);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
  // field.name toll_mode
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->toll_mode.size + 1);
  // field.name toll_lane_type
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->toll_lane_type.size + 1);
  // field.name toll_name
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->toll_name.size + 1);
  // field.name restriction_detail

  current_alignment += get_serialized_size_deva_gaode_routing_msgs__msg__RestrictionDetail(
    &(ros_message->restriction_detail), current_alignment);

  return current_alignment - initial_alignment;
}

static uint32_t _Restriction__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_deva_gaode_routing_msgs__msg__Restriction(
      untyped_ros_message, 0));
}

ROSIDL_TYPESUPPORT_FASTRTPS_C_PUBLIC_deva_gaode_routing_msgs
size_t max_serialized_size_deva_gaode_routing_msgs__msg__Restriction(
  bool & full_bounded,
  bool & is_plain,
  size_t current_alignment)
{
  size_t initial_alignment = current_alignment;

  const size_t padding = 4;
  const size_t wchar_size = 4;
  size_t last_member_size = 0;
  (void)last_member_size;
  (void)padding;
  (void)wchar_size;

  full_bounded = true;
  is_plain = true;

  // member: id
  {
    size_t array_size = 1;

    last_member_size = array_size * sizeof(uint64_t);
    current_alignment += array_size * sizeof(uint64_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint64_t));
  }
  // member: limit_type
  {
    size_t array_size = 1;

    last_member_size = array_size * sizeof(uint32_t);
    current_alignment += array_size * sizeof(uint32_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint32_t));
  }
  // member: node_id
  {
    size_t array_size = 1;

    last_member_size = array_size * sizeof(uint64_t);
    current_alignment += array_size * sizeof(uint64_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint64_t));
  }
  // member: pass_list
  {
    size_t array_size = 0;
    full_bounded = false;
    is_plain = false;
    current_alignment += padding +
      eprosima::fastcdr::Cdr::alignment(current_alignment, padding);

    last_member_size = array_size * sizeof(uint64_t);
    current_alignment += array_size * sizeof(uint64_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint64_t));
  }
  // member: out_road
  {
    size_t array_size = 1;

    last_member_size = array_size * sizeof(uint64_t);
    current_alignment += array_size * sizeof(uint64_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint64_t));
  }
  // member: passage
  {
    size_t array_size = 1;

    last_member_size = array_size * sizeof(uint32_t);
    current_alignment += array_size * sizeof(uint32_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint32_t));
  }
  // member: slope
  {
    size_t array_size = 1;

    last_member_size = array_size * sizeof(uint32_t);
    current_alignment += array_size * sizeof(uint32_t) +
      eprosima::fastcdr::Cdr::alignment(current_alignment, sizeof(uint32_t));
  }
  // member: toll_mode
  {
    size_t array_size = 1;

    full_bounded = false;
    is_plain = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: toll_lane_type
  {
    size_t array_size = 1;

    full_bounded = false;
    is_plain = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: toll_name
  {
    size_t array_size = 1;

    full_bounded = false;
    is_plain = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: restriction_detail
  {
    size_t array_size = 1;


    last_member_size = 0;
    for (size_t index = 0; index < array_size; ++index) {
      bool inner_full_bounded;
      bool inner_is_plain;
      size_t inner_size;
      inner_size =
        max_serialized_size_deva_gaode_routing_msgs__msg__RestrictionDetail(
        inner_full_bounded, inner_is_plain, current_alignment);
      last_member_size += inner_size;
      current_alignment += inner_size;
      full_bounded &= inner_full_bounded;
      is_plain &= inner_is_plain;
    }
  }

  size_t ret_val = current_alignment - initial_alignment;
  if (is_plain) {
    // All members are plain, and type is not empty.
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = deva_gaode_routing_msgs__msg__Restriction;
    is_plain =
      (
      offsetof(DataType, restriction_detail) +
      last_member_size
      ) == ret_val;
  }

  return ret_val;
}

static size_t _Restriction__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_deva_gaode_routing_msgs__msg__Restriction(
    full_bounded, is_plain, 0);

  bounds_info =
    is_plain ? ROSIDL_TYPESUPPORT_FASTRTPS_PLAIN_TYPE :
    full_bounded ? ROSIDL_TYPESUPPORT_FASTRTPS_BOUNDED_TYPE : ROSIDL_TYPESUPPORT_FASTRTPS_UNBOUNDED_TYPE;
  return ret_val;
}


static message_type_support_callbacks_t __callbacks_Restriction = {
  "deva_gaode_routing_msgs::msg",
  "Restriction",
  _Restriction__cdr_serialize,
  _Restriction__cdr_deserialize,
  _Restriction__get_serialized_size,
  _Restriction__max_serialized_size
};

static rosidl_message_type_support_t _Restriction__type_support = {
  rosidl_typesupport_fastrtps_c__identifier,
  &__callbacks_Restriction,
  get_message_typesupport_handle_function,
};

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, deva_gaode_routing_msgs, msg, Restriction)() {
  return &_Restriction__type_support;
}

#if defined(__cplusplus)
}
#endif
