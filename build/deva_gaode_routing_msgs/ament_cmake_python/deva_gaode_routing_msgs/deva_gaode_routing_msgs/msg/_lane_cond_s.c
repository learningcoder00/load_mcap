...
bool deva_gaode_routing_msgs__msg__lane_cond__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("deva_gaode_routing_msgs.msg._lane_cond.LaneCond", full_classname_dest, 47) == 0);
  }
  deva_gaode_routing_msgs__msg__LaneCond * ros_message = _ros_message;
  {  // laneinfo_cond
  ...
  {  // vehicles
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      uint8_t * dest = ros_message->vehicles.data;
  ...
  {  // special_times
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      uint8_t * dest = ros_message->special_times.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'special_times'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of LaneCond */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("deva_gaode_routing_msgs.msg._lane_cond");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "LaneCond");
  ...
  {  // vehicles
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "vehicles");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->vehicles.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint8_t * src = &(ros_message->vehicles.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->vehicles.size * sizeof(uint8_t));
      assert(data != NULL);
  ...
  {  // special_times
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "special_times");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->special_times.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint8_t * src = &(ros_message->special_times.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->special_times.size * sizeof(uint8_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
