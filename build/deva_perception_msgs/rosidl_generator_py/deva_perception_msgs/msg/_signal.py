# generated from rosidl_generator_py/resource/_idl.py.em
# with input from deva_perception_msgs:msg/Signal.idl
# generated code does not contain a copyright notice


# Import statements for member types

import builtins  # noqa: E402, I100

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_Signal(type):
    """Metaclass of message 'Signal'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('deva_perception_msgs')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'deva_perception_msgs.msg.Signal')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__signal
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__signal
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__signal
            cls._TYPE_SUPPORT = module.type_support_msg__msg__signal
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__signal

            from deva_perception_msgs.msg import StaticObstacle
            if StaticObstacle.__class__._TYPE_SUPPORT is None:
                StaticObstacle.__class__.__import_type_support__()

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class Signal(metaclass=Metaclass_Signal):
    """Message class 'Signal'."""

    __slots__ = [
        '_so',
        '_signal_type',
        '_geometry_type',
        '_state_type',
    ]

    _fields_and_field_types = {
        'so': 'deva_perception_msgs/StaticObstacle',
        'signal_type': 'uint8',
        'geometry_type': 'uint8',
        'state_type': 'uint8',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['deva_perception_msgs', 'msg'], 'StaticObstacle'),  # noqa: E501
        rosidl_parser.definition.BasicType('uint8'),  # noqa: E501
        rosidl_parser.definition.BasicType('uint8'),  # noqa: E501
        rosidl_parser.definition.BasicType('uint8'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        from deva_perception_msgs.msg import StaticObstacle
        self.so = kwargs.get('so', StaticObstacle())
        self.signal_type = kwargs.get('signal_type', int())
        self.geometry_type = kwargs.get('geometry_type', int())
        self.state_type = kwargs.get('state_type', int())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.so != other.so:
            return False
        if self.signal_type != other.signal_type:
            return False
        if self.geometry_type != other.geometry_type:
            return False
        if self.state_type != other.state_type:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def so(self):
        """Message field 'so'."""
        return self._so

    @so.setter
    def so(self, value):
        if __debug__:
            from deva_perception_msgs.msg import StaticObstacle
            assert \
                isinstance(value, StaticObstacle), \
                "The 'so' field must be a sub message of type 'StaticObstacle'"
        self._so = value

    @builtins.property
    def signal_type(self):
        """Message field 'signal_type'."""
        return self._signal_type

    @signal_type.setter
    def signal_type(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'signal_type' field must be of type 'int'"
            assert value >= 0 and value < 256, \
                "The 'signal_type' field must be an unsigned integer in [0, 255]"
        self._signal_type = value

    @builtins.property
    def geometry_type(self):
        """Message field 'geometry_type'."""
        return self._geometry_type

    @geometry_type.setter
    def geometry_type(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'geometry_type' field must be of type 'int'"
            assert value >= 0 and value < 256, \
                "The 'geometry_type' field must be an unsigned integer in [0, 255]"
        self._geometry_type = value

    @builtins.property
    def state_type(self):
        """Message field 'state_type'."""
        return self._state_type

    @state_type.setter
    def state_type(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'state_type' field must be of type 'int'"
            assert value >= 0 and value < 256, \
                "The 'state_type' field must be an unsigned integer in [0, 255]"
        self._state_type = value
