...
bool deva_perception_msgs__msg__lane_line2d__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("deva_perception_msgs.msg._lane_line2d.LaneLine2d", full_classname_dest, 48) == 0);
  }
  deva_perception_msgs__msg__LaneLine2d * ros_message = _ros_message;
  {  // lane_id
  ...
  {  // lane_color
    ...
    assert(PyLong_Check(field));
    ros_message->lane_color = (uint8_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // lane_property
  ...
  {  // curve_line_coeffs
    PyObject * field = PyObject_GetAttrString(_pymsg, "curve_line_coeffs");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'curve_line_coeffs'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!deva_perception_msgs__msg__CurveCoef__Sequence__init(&(ros_message->curve_line_coeffs), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create deva_perception_msgs__msg__CurveCoef__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    deva_perception_msgs__msg__CurveCoef * dest = ros_message->curve_line_coeffs.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!deva_perception_msgs__msg__curve_coef__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of LaneLine2d */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("deva_perception_msgs.msg._lane_line2d");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "LaneLine2d");
  ...
  {  // lane_color
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->lane_color);
    {
  ...
  {  // curve_line_coeffs
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->curve_line_coeffs.data[i]);
      PyObject * pyitem = deva_perception_msgs__msg__curve_coef__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    assert(PySequence_Check(field));
    {
      int rc = PyObject_SetAttrString(_pymessage, "curve_line_coeffs", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
